...
@section('content')
    ...
    <div class="cert-grid">
        ...
        {{-- Tombol Add --}}
        <a href="{{ route('admin.sertifikat.create') }}" class="add-cert-card">
            <div class="add-cert-icon">
                <i class="bi bi-plus-lg"></i>
            </div>
            <span class="add-cert-label">Tambah Sertifikat</span>
        </a>

        {{-- Certificate Cards --}}
        @forelse ($certificates as $cert)
            <div class="cert-card" style="background: linear-gradient(135deg, #1e3a5f, #2563eb);">
                ...
                <div class="cert-content">
                    ...
                    {{-- Top: category badge --}}
                    <div class="cert-top">
                        <span class="cert-cat-badge cert-cat-blue">
                            {{ $cert->sertifikat }} {{-- kategori --}}
                        </span>
                    </div>

                    {{-- Bottom: title + edit & hapus --}}
                    <div class="cert-bottom">
                        <div class="cert-title">{{ $cert->legalitas }}</div> {{-- nama sertifikat --}}

                        <div class="cert-footer">
                            {{-- Tombol Edit --}}
                            <a href="{{ route('admin.sertifikat.edit', $cert->id_dok_perusahaan) }}" class="cert-action-btn"
                                title="Edit">
                                <i class="bi bi-pencil"></i>
                            </a>

                            {{-- Tombol Hapus --}}
                            <form method="POST" action="{{ route('admin.sertifikat.destroy', $cert->id_dok_perusahaan) }}">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="cert-action-btn" title="Hapus"
